...
const TopOffset0 = 125;//225;
const TopOffset1 = 85;//185;
...
function launchEditor() {
    if (inviting) {
        $(".signer-save span").text("Send");
    } else {
        $(".signer-save span").text("Save");
    }
...
function sendRequest() {
    ...
    if ($("input[name=duplicate]").prop("checked") && emails.length > 1) {
        duplicate = "Yes";
    }
    if ($("input[name=chain]").prop("checked") && emails.length > 1) {
        chain = "Yes";
    }
...
$(".signer-tool").click(function (event) {
    ...
    if (tool !== "rotate" && $('action[type=rotate]').length) {
        toastr.warning("Save rotation changes before editing document.", "Hmm!", {
            timeOut: 2000,
            closeButton: true,
            progressBar: false
        });
    ...
    if (tool === "rotate") {
        if ($('.signer-builder .signer-element').length || modules.original !== $('#document-viewer').getCanvasImage("image/png")) {
            toastr.warning("Save changes before rotating.", "Hmm!", {
                timeOut: 2000,
                closeButton: true,
                progressBar: false
            })
...
function assemble(data, prepare) {
    ...
    } else if (data.type === "image" || data.type === Initials || data.type === "signature" || data.type === "symbol" || data.type === "shape" || data.type === "stamp") {
        if (data.group === "field") {
...
function acceptRequest() {
    ...
    if ($(".signer-builder").is(':empty')) {
        ...
        if ($("body").hasClass("accept") && requestPositions.length) {
            showLoader();
            if ($(window).width() < 1101) {
                topOffset = TopOffset0;
            } else {
                topOffset = TopOffset1;
            }
            currentOffset = $(".signer-overlay-previewer").offset();
            currentDocOffset = $("#document-viewer").offset();
            currentPosition = $("#document-viewer").position();
